...
def _check_utc_offset(name, offset):
    ...
    if offset.__mod__(timedelta(minutes=1)) or offset.microseconds:
...
class datetime(date):
    ...
    def fromtimestamp(cls, t, tz=None):
        ...
        if us == 1000000:
            t += 1
            us = 0
        y, m, d, hh, mm, ss, weekday, jday, dst = converter(int(t))
    ...
    def utcfromtimestamp(cls, t):
        ...
        if us == 1000000:
            t += 1
            us = 0
        y, m, d, hh, mm, ss, weekday, jday, dst = _time.gmtime(int(t))
        ss = min(ss, 59)  # clamp out leap seconds if the platform has them
...
#            assert not mm % timedelta(minutes=1), "whole minute"
#            mm //= timedelta(minutes=1)
            hh, mm = off.__divmod__(timedelta(hours=1))
            assert not mm.__mod__(timedelta(minutes=1)), "whole minute"
            mm = mm.__floordiv__(timedelta(minutes=1))
